        ...
        >
          ...
          <template v-if="viewMode === 'list'">
            ...
            <div class="record-content">
              <div class="content-preview">{{ getContentPreview(record.content, 'list') }}</div>
            </div>
          </template>
          ...
          <template v-else>
            ...
            <div class="grid-content">
              <div class="content-preview" v-html="parseMarkdown(getContentPreview(record.content, 'grid'))"></div>
            </div>
          </template>
        </div>
...
// 获取内容预览
const getContentPreview = (content, mode = 'list') => {
  if (!content) return ''

  if (mode === 'list') {
    // 列表模式：移除Markdown符号，限制字数
    const plainText = removeMarkdown(content)
    return plainText.length > 100 ? plainText.substring(0, 100) + '...' : plainText
  } else {
    // 平铺模式：保留Markdown格式，不限制字数
    return content
  }
};
